    ...
    {#each items as item}
      ...
      <article class="flex min-h-72 flex-col border-b border-ink py-6 pr-4 sm:pr-7">
        <div class="relative aspect-[4/3] overflow-hidden bg-stone">
          <img class="h-full w-full object-cover" src={illustration.src} alt={`${illustration.alt} — foto ilustrasi sementara, bukan dokumentasi proyek HD Karya Bandung`} width="1448" height="1086" loading="lazy" />
...
{:else}
  <section class="grid overflow-hidden border border-stone bg-mist md:grid-cols-[0.85fr_1.15fr]">
    ...
    <div class="p-8 sm:p-12"><p class="eyebrow text-clay">Sedang disiapkan</p><h2 class="mt-4 font-display text-4xl font-bold tracking-[-0.04em]">{emptyTitle}</h2><p class="mt-4 max-w-2xl leading-7 text-forest">{emptyBody}</p><p class="mt-5 text-sm leading-6 text-forest">Foto ilustrasi sementara — bukan dokumentasi proyek HD Karya Bandung.</p><a class="button-primary mt-7" href="/kontak">Konsultasikan kebutuhan Anda</a></div>
  </section>
{/if}
