...
import { portfolioCategories } from '$lib/server/cms';
...
export const GET: RequestHandler = async ({ url, platform }) => {
  ...
  const entries = [
    ...fixed.map((path) => ({ path, lastmod: today })),
    ...portfolioCategories.map((category) => ({ path: `/portofolio/${category.slug}`, lastmod: today })),
    ...rows.results.map((row) => ({ path: `/${routeForKind(row.kind)}/${row.slug}`, lastmod: row.updated_at.slice(0, 10) }))
  ];
